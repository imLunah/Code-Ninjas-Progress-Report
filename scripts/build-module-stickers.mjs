...
function build() {
  ...
  const picked = keys.map((key) => {
    ...
    return { key, program, subProgram: subProgram || null, moduleName, title, file: hit.file, id: slug(`${program}-${subProgram}-${moduleName}`) };
  });
...
function dataFile(picked) {
  ...
  for (const p of picked) {
    ...
    lines.push(`  m(${JSON.stringify(p.program)}, ${JSON.stringify(p.subProgram)}, ${JSON.stringify(p.moduleName)}, ${JSON.stringify(p.title)}),`);
  }
