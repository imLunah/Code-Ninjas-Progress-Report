...
function LocationBar({ user, switchLocation, compact }) {
  const glass = `rounded-xl border border-white/15 dark:border-white/10 bg-white/5 dark:bg-[#141826]/30 backdrop-blur-xl backdrop-saturate-150 text-ninja-navy font-ninja font-semibold shadow-lg shadow-black/25 transition-all duration-200 ${compact ? 'px-2.5 py-1 text-xs' : 'px-3 py-1.5 text-sm'}`;
...
export default function MobileNav({ compact = false }) {
  ...
  return (
    <nav className="lg:hidden absolute bottom-0 inset-x-0 z-20 px-2 pb-[max(env(safe-area-inset-bottom),8px)] pt-2 pointer-events-none flex flex-col items-center gap-1.5">
      ...
      <div
        className={`pointer-events-auto relative flex items-center gap-0.5 rounded-full border border-white/15 dark:border-white/10 bg-white/5 dark:bg-[#141826]/30 backdrop-blur-xl backdrop-saturate-150 transition-all duration-200 ${compact ? 'px-1.5 py-1' : 'px-2 py-1.5'}`}
