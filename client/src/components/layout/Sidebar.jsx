      ...
      {user && (
        <div className="px-3 pt-3">
          {user.role === 'manager' ? (
            ...
            <div className="text-ninja-muted text-sm font-ninja px-3 py-2 bg-ninja-bg border border-ninja-border rounded-lg truncate">
      ...
      <nav className="flex-1 p-3 space-y-0.5 mt-2 overflow-y-auto">
        {navLinks.map((link) => {
          const isActive = location.pathname === link.to ||
            (link.to.length > 1 && location.pathname.startsWith(link.to + '/'));
          return (
            <Link
              key={link.to}
              to={link.to}
      ...
      <div className="p-3 border-t border-ninja-border">
        <div className="flex items-center gap-2.5 px-2 py-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="font-ninja text-ninja-muted text-xs capitalize">{user?.role}</p>
          </div>
          <button
