...
export function Hero({ program, size = 'card', className = '', style = {}, children }) {
  ...
  // top, edge to edge, no gap above it, with all four corners rounded like
  // the screen itself. On desktop it is a card in the right column like
  // everything else.
  const pad = size === 'page'
    ? '-mx-4 sm:-mx-6 -mt-5 rounded-[34px] px-6 pt-[max(1.5rem,env(safe-area-inset-top))] pb-6 lg:mx-0 lg:mt-0 lg:rounded-[26px] lg:px-7 lg:pt-7 lg:pb-6'
    : 'p-4 rounded-[18px]';
  return (
